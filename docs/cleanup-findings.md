...
These findings were recorded during the PR #116 cleanup. **No bug fixes or
runtime code/data removals were made for these findings.** Module moves, type
definitions and explanatory comments are separate from the decisions below.

## Suspected bugs and inconsistencies

| Finding | Evidence and effect | Suggested follow-up |
| --- | --- | --- |
| Scenario ID `0` skips update guards | In [the update route](../backend/app/routers/scenarios.py), `if scenario_id:` excludes valid ID `0`. It bypasses editability, revision and map/JSON propagation checks before reaching `update_scenario`. | Add a focused ID-0 API regression, then check presence explicitly. Review other truthiness checks on scenario IDs in the same fix. |
| Workshop SRA diagram filename casing | [check_for_diagram](../backend/app/internal/scenario_handler.py) builds `workshop_sra_input.png` / `workshop_sra_output.png`; the files are named `workshop_SRA_input.png` / `workshop_SRA_output.png`. | Verify on a case-sensitive filesystem and standardize the lookup. |
